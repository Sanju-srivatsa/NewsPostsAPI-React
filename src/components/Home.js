    ...
    return (<div id={activeFeedback?'hero':'villain'}>
            ...
            {layout&&(finalOutput&&finalOutput.map((ele,i)=>{
                return(<>
                            ...
                            <div><img id='image'src={`https://picsum.photos/200/200?random=${i+1}`} alt=""/></div>
...
(!layout)&&(finalOutput&&finalOutput.map((ele,i)=>{
    ...
    <div  onClick={()=>setFrame(true)} ><b><p>{ele.title}</p></b><p>{ele.body}</p><img id='image1'src={`https://picsum.photos/200/200?random=${i+1}`} alt=""/></div></div></>
